...
import (
	...
	"github.com/ac0mz/proglog/internal/agent"
	"github.com/ac0mz/proglog/internal/config"
	"github.com/spf13/cobra"
	"github.com/spf13/viper"
)

// k8sでサービスを実行するための、Dockerイメージにおけるエントリーポイント
func main() {
	cli := &cli{}
...
// cli は全コマンドに共通するロジックやデータを保持する。
type cli struct {
	cfg cfg
}

// cfg はエラー処理が必須となるフィールドの型を保持する。
type cfg struct {
	agent.Config
...
func setupFlags(cmd *cobra.Command) error {
	...
	cmd.Flags().Int("prc-port", 8400, "Port for RPC clients (and Raft) connections.")
...
// setupConfig は設定を読み込み、エージェントの設定を準備する。
func (c *cli) setupConfig(cmd *cobra.Command, args []string) error {
	...
	c.cfg.RPCPort = viper.GetInt("prc-port")
	...
	c.cfg.ACLModelFile = viper.GetString("acl-policy-file")
...
// run は次の実行ファイルのロジックを実行する。
//   - エージェント作成
//   - OSからのシグナルを処理
//   - OSがプログラムを終了させる場合、エージェントをグレースフルシャットダウンさせる
